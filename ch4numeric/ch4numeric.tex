...
已知$n+1$个不同点$(x_i,y_i),x_i\neq x_j,1\leq i \leq j\leq n$,则所求的多项式$n$次多项式$P_n(x)=a_0+a_1x+\cdots+a_nx^n$是唯一的。
\end{theorem}
...
\right.
$$
这个方程组写成矩阵的形式可以知道，其矩阵是一个范德蒙矩阵，因此非奇异，所以有唯一解，可以利用克莱姆法则解得唯一的一组系数$a_0,a_1,\cdots,a_n$。
\subsection{插值问题的实质}
...
除了上面待定系数的方法以外，牛顿插值的系数$c_k$还可以利用差商来求解，$x_i$处的0阶差商$f[x_i]\equiv y_i$,$x_i$处的1阶差商$f[x_i,x_{i+1}]\equiv \frac{f[x_{i+1}]-f[x_i]}{x_{i+1}-x_i}$,$x_i$处的$k$阶差商由两个低阶差商定义，$f[x_i,\cdots,x_{k}]\equiv\frac{f[x_{i+1},\cdots,x_{k}]-f[x_i,\cdots,x_{k-1}]}{x_{i+1}-x_i}$.
上面系数
